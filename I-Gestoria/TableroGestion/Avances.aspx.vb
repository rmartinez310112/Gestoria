...
Partial Class TableroGestion_Avances
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            '    cargaClientes(Request.QueryString("sesionClienteTablero"))
            '    cargaServicioTipo(Request.QueryString("sesionClienteTablero"))
            'Else
            '    cargaClientes()
            '    cargaServicioTipo()
            'End If
            ...
            TotalesAvance1.Visible = False
            LlenaFiltros()
        End If
    End Sub
    Private Function LlenaTotales(ByVal DT As DataTable)
        Dim procesoTotal As LinkButton = TotalesAvance1.FindControl("lblTerminados")

        Dim antesTiempo As LinkButton = TotalesAvance1.FindControl("lblAntes")
        Dim antesTiempoPorc As Label = TotalesAvance1.FindControl("lblAntesP")
        Dim enTiempo As LinkButton = TotalesAvance1.FindControl("lblTiempo")
        Dim enTiempoPorc As Label = TotalesAvance1.FindControl("lblTiempoP")
        Dim fueraTiempo As LinkButton = Me.TotalesAvance1.FindControl("lblFuera")
        Dim fueraTiempoPorc As Label = TotalesAvance1.FindControl("lblFueraP")

        procesoTotal.Text = DT.Compute("Sum(terminados)", String.Empty)

        antesTiempo.Text = DT.Compute("Sum(Antes_de_Tiempo)", String.Empty)
        antesTiempoPorc.Text = String.Format("{0:P0}", (Convert.ToDecimal(antesTiempo.Text) / Convert.ToDecimal(procesoTotal.Text)))

        enTiempo.Text = DT.Compute("Sum(En_Tiempo)", String.Empty)
        enTiempoPorc.Text = String.Format("{0:P0}", (Convert.ToDecimal(enTiempo.Text) / Convert.ToDecimal(procesoTotal.Text)))

        fueraTiempo.Text = DT.Compute("Sum(Fuera_de_Tiempo)", String.Empty)
        fueraTiempoPorc.Text = String.Format("{0:P0}", (Convert.ToDecimal(fueraTiempo.Text) / Convert.ToDecimal(procesoTotal.Text)))
        ...
        If (RadComboBox3.SelectedValue = 1) Then
            ...
            procesoTotal.Attributes.Add("onclick", "window.open('D4.aspx?IdCliente=" + CboCliente.SelectedValue + "&IdServicio=" + cboServicioTipo.SelectedValue + "&IdRegion=" + cboRegion.SelectedValue + "&IdEstado=" + cboEstado.SelectedValue + "&FechaI=" + FechaI + "&FechaF=" + FechaF + "&sesionClienteTablero=" + csTC.obtieneClienteTableto(Request.QueryString("sesionClienteTablero"), CStr(cbocliente.SelectedValue)) + "', 'newwindow','toolbar=yes,location=no,menubar=no,width=1300,height=700,resizable=yes,scrollbars=yes,top=200,left=250');return true;")
            antesTiempo.Attributes.Add("onclick", "window.open('D5.aspx?IdCliente=" + CboCliente.SelectedValue + "&IdServicio=" + cboServicioTipo.SelectedValue + "&IdRegion=" + cboRegion.SelectedValue + "&IdEstado=" + cboEstado.SelectedValue + "&FechaI=" + FechaI + "&FechaF=" + FechaF + "&sesionClienteTablero=" + csTC.obtieneClienteTableto(Request.QueryString("sesionClienteTablero"), CStr(cbocliente.SelectedValue)) + "', 'newwindow','toolbar=yes,location=no,menubar=no,width=1300,height=700,resizable=yes,scrollbars=yes,top=200,left=250');return true;")
            enTiempo.Attributes.Add("onclick", "window.open('D6.aspx?IdCliente=" + CboCliente.SelectedValue + "&IdServicio=" + cboServicioTipo.SelectedValue + "&IdRegion=" + cboRegion.SelectedValue + "&IdEstado=" + cboEstado.SelectedValue + "&FechaI=" + FechaI + "&FechaF=" + FechaF + "&sesionClienteTablero=" + csTC.obtieneClienteTableto(Request.QueryString("sesionClienteTablero"), CStr(cbocliente.SelectedValue)) + "', 'newwindow','toolbar=yes,location=no,menubar=no,width=1300,height=700,resizable=yes,scrollbars=yes,top=200,left=250');return true;")
            fueraTiempo.Attributes.Add("onclick", "window.open('D7.aspx?IdCliente=" + CboCliente.SelectedValue + "&IdServicio=" + cboServicioTipo.SelectedValue + "&IdRegion=" + cboRegion.SelectedValue + "&IdEstado=" + cboEstado.SelectedValue + "&FechaI=" + FechaI + "&FechaF=" + FechaF + "&sesionClienteTablero=" + csTC.obtieneClienteTableto(Request.QueryString("sesionClienteTablero"), CStr(cbocliente.SelectedValue)) + "', 'newwindow','toolbar=yes,location=no,menubar=no,width=1300,height=700,resizable=yes,scrollbars=yes,top=200,left=250');return true;")
    ...
    Protected Sub radBtnResultado_Click(sender As Object, e As System.EventArgs) Handles radBtnResultado.Click
        ...
        ds = csDAL.CargaTableroAvance(cliente, servicio_tipo, FechaI, FechaF, IDmes, region, estado)
        dt = ds.Tables(0)
    ...
    Private Sub LlenaFiltros()
        ...
        If (IdServicio <> "0") And (IdServicio <> "0") Then
